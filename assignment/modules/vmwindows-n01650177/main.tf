...
resource "azurerm_windows_virtual_machine" "win_vm" {
  count               = 1
  name                = "n01650177-w-vm${count.index + 1}"
...
resource "azurerm_network_interface" "win_vm_nic" {
  count               = 1
  name                = "n01650177-w-vm${count.index + 1}-nic"
  location            = var.location
  resource_group_name = var.resource_group_name
...
resource "azurerm_public_ip" "win_vm_pip" {
  count               = 1
  name                = "n01650177-w-vm${count.index + 1}-pip"
  resource_group_name = var.resource_group_name
  location            = var.location
  allocation_method   = "Dynamic"
  domain_name_label   = "n01650177-w-vm${count.index + 1}"
}
